...
from Processing.WildberriesReports.Headers.TableHeaders import wildberries_stocks_headers_list
...
log = get_logger("ReportStocks")


class ReportStocks(WildberriesProcessingMethods):
    ...
    def __url_builder():
        ...
        if app_mode == 'prod':
            ...
            date_config = read_config('wildberries.stocksreport').get('reportdate')
            time_config = read_config('wildberries.stocksreport').get('reporttime')
            log.info(f"Report datetime {' '.join([date_config, time_config])}")
            date = datetime.strptime(' '.join([date_config, time_config]), '%d.%m.%Y %H:%M')
            date_form = f'{date.year:04d}-{date.month:02d}-{date.day:02d}'
    ...
    @staticmethod
    def __get_headers():
        return wildberries_stocks_headers_list
