...
from Processing.WildberriesReports.Headers.TableHeaders import wildberries_incomes_headers_list
...
log = get_logger("ReportIncomes")


class ReportIncomes(WildberriesProcessingMethods):
    ...
    def __url_builder():
        ...
        if app_mode == 'prod':
            ...
            date_config = read_config('wildberries.incomesreport').get('reportdate')
            time_config = read_config('wildberries.incomesreport').get('reporttime')
    ...
    @staticmethod
    def __get_headers():
        return wildberries_incomes_headers_list
